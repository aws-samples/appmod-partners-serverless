...
#        print(f"extension.logs_api_http_extension: Starting HTTP Server {agent_name}")
    ...
    def run_forever(self):
        ...
        failure_s3_bucket = (os.environ['FAILED_S3_BUCKET_NAME'])
        success_s3_bucket= (os.environ['SUCCESS_S3_BUCKET_NAME'])
...
##                        response = s3.Bucket(success_s3_bucket).put_object(Key=s3_filename, Body=content)
#                except Exception as e:
#                    raise Exception(f"Error sending log to S3 {e}") from e



#               Code for sending logs to different buckets
                try:
                   for item in range(len(batch)):
                        content = batch[item]
                        print(content)
                        if content['type'] == 'function':
                            if "ERROR" in content['record']:
                                s3_filename = (os.environ['AWS_LAMBDA_FUNCTION_NAME'])+'-'+'error-file'+ (datetime.now()).strftime('%Y-%m-%d-%H:%M:%S.%f')+'.'+str(item)+'.log'
                                response= s3.Bucket(failure_s3_bucket).put_object(Key=s3_filename, Body= str(content))
                            else:
                                s3_filename = (os.environ['AWS_LAMBDA_FUNCTION_NAME'])+'-'+'log-file'+(datetime.now()).strftime('%Y-%m-%d-%H:%M:%S.%f')+'.'+str(item)+'.log'
                                response = s3.Bucket(success_s3_bucket).put_object(Key=s3_filename, Body=str(content))
                        else:
                            s3_filename = (os.environ['AWS_LAMBDA_FUNCTION_NAME'])+'-'+'log-file'+(datetime.now()).strftime('%Y-%m-%d-%H:%M:%S.%f')+'.'+str(item)+'.log'
                            response = s3.Bucket(success_s3_bucket).put_object(Key=s3_filename, Body=str(content))
                except Exception as e:
